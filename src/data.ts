...
// ============================================================
// INTERFACE – šablony pro datové objekty
// Interface říká TypeScriptu "každý objekt tohoto typu musí mít přesně tyto vlastnosti".
// Pokud bychom zapomněli třeba na "vaha", TypeScript by okamžitě ukázal chybu.
// ============================================================

// Šablona pro surová data zbraní
interface RawZbran {
    id: string;                  // Unikátní kód (např. "zbr001")
    nazev: string;               // Zobrazovaný název
    popis: string;               // Popis předmětu (zobrazuje se v detailu)
    typ: string;                 // Druh zbraně: "meč", "dýka", "luk"...
    poskozeni: number;           // Základní poškození
    rychlost: number;            // Rychlost útoku
    multiplikatorRarity: number; // Koeficient vzácnosti (1.0 = běžná, 3.0 = legendární)
    rarity: string;              // Slovní popis vzácnosti
    vaha: number;                // Váha v kg
}

// Šablona pro surová data brnění
interface RawBrneni {
    id: string;
    nazev: string;
    popis: string;
    typ: string;                 // "kožená zbroj", "plášť", "šupiny"...
    obrana: number;              // Hodnota obrany (ovlivňuje max HP hráče)
    rychlost: number;            // Záporné číslo = zpomalení hráče
    multiplikatorRarity: number;
    rarity: string;
    vaha: number;
}

// Šablona pro surová data lektvarů
interface RawLektvar {
    id: string;
    nazev: string;
    popis: string;
    typ: string;
    efekt: string;               // Co se stane po použití: "obnova zdraví", "jed"...
    trvaniEfektu: number;        // 0 = okamžitý účinek, >0 = trvá X sekund
    multiplikatorRarity: number;
    rarity: string;
    vaha: number;
}

// Šablona pro surová data svitků (stejná struktura jako lektvar)
interface RawSvitek {
    id: string;
    nazev: string;
    popis: string;
...
// Šablona pro crafting recepty
// Každý recept říká: "kombinuj ingredience1 + ingredience2 → vysledekId"
interface CraftingRecept {
    id: string;           // Unikátní ID receptu
    ingredience1: string; // ID prvního předmětu (musí být v inventáři)
    ingredience2: string; // ID druhého předmětu (musí být v inventáři)
    vysledekId: string;   // ID výsledného předmětu (přidá se do inventáře)
    nazev: string;        // Název výsledku (pro zobrazení)
    popis: string;        // Popis receptu (pro zobrazení)
}
